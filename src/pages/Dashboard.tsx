    ...
    <div className="flex h-screen bg-background">
      ...
      <aside className="w-64 bg-primary text-primary-foreground p-4">
        <div className="flex items-center space-x-2 mb-6">
          ...
          <div>
            <h2 className="font-semibold">{user?.displayName || 'God'}</h2>
            <p className="text-sm">{user?.email || 'luffy@pirateking.com'}</p>
          </div>
        </div>
      ...
      <main className="flex-1 p-8 overflow-auto">
        <h1 className="text-3xl font-bold mb-6">Public Posts</h1>
        <Card>
          <CardContent>
            <ScrollArea className="h-[calc(100vh-200px)] mt-6">
              <div className="space-y-6">
                {posts.map((post) => (
                  ...
                  <div key={post.id} className="flex items-start space-x-4 p-4 bg-secondary rounded-lg">
